...
if __name__ == "__main__":
    extract = Extract()
    df = extract.recordPathwithID(save=True)

    # standardize: False, True, ‘zscore’, ‘psc’
    # Strategy to standardize the signal.
    # 'zscore': the signal is z-scored.
    # Timeseries are shifted to zero mean and scaled to unit variance.
    # 'psc': Timeseries are shifted to zero mean value and scaled to percent signal change
    # (as compared to original mean signal).
    # True : the signal is z-scored.
    # Timeseries are shifted to zero mean and scaled to unit variance.
    ...
    transform = Transform(standardize=standardize)
    transform.multiproc_multithread_extractTimeSeries(
        df["filePath"].values, save_npy, nproc, max_workers
    )

    load = Load()
    data, labels = load.loadSiteData(site=[1, 3, 4, 5, 6])
