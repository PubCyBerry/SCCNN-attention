...
@dataclass
class LOSODataModule(BaseKFoldDataModule):
    ...
    def setup_fold_index(self, fold_index: int) -> None:
        ...
        # test_site = sites.pop((fold_index + 4) % 5)
        test_site = sites.pop(fold_index)
        self.train_fold = ConcatDataset([self.usr_dataset(site) for site in sites])
        self.val_fold = self.usr_dataset(test_site)

        print(f"TRAIN FOLD: {fold_index + 1}, {len(self.train_fold)}")
        print(f"VALID FOLD: {fold_index + 1}, {len(self.val_fold)}")
        print("TEST SITE: {}".format(re.search(r'\w*_(.*?)(?=\.)', test_site).group(1)))
    ...
    def val_dataloader(self) -> DataLoader:
        return DataLoader(self.val_fold, **self.loader.eval, collate_fn=collate_fn)
...
class LOSOTestModel(LightningModule):
    def __init__(self, model_cls: Type[LightningModule], checkpoint_paths: List[str]):
        ...
        # )
        self.model = model_cls.load_from_checkpoint(checkpoint_paths[0])
    ...
    def test_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> None:
        ...
        logit = self.model(batch[0])
        acc = self.test_acc(logit, batch[1])
        ...
        self.log_dict({f"final_result": acc}, prog_bar=True)
